...
import { getPanelById, PANELS } from "./panels.registry";
            ...
            children: [
              {
                ...
                weight: 50,
                children: [
                  { type: "tab", name: "Task Detail", component: "task-detail" },
                ],
              },
              {
                type: "tabset",
                weight: 50,
                children: [
                  { type: "tab", name: "Agent", component: "agent-thread" },
                ],
              },
            ],
          },
          {
            ...
            children: [
              ...
              },
              {
                ...
                weight: 25,
                children: [
                  { type: "tab", name: "Git", component: "git-panel" },
                ],
              },
              {
                type: "tabset",
                weight: 25,
                children: [
                  { type: "tab", name: "Actions", component: "action-panel" },
                ],
              },
            ],
          },
...
function loadModel(workspaceId: string | null): Model {
  try {
    // Try workspace-specific first, fall back to legacy key
    const key = getLayoutKey(workspaceId);
    const saved = localStorage.getItem(key) ?? (workspaceId ? localStorage.getItem("actly:layout") : null);
...
function getPanelComponent(id: string) {
  if (!panelComponents[id]) {
    ...
    if (def) {
      panelComponents[id] = lazy(def.component);
    }
  }
...
export default function WorkspaceLayout() {
  ...
  // Focus or add a panel by component ID
  const activatePanel = useCallback((panelId: string) => {
    ...
    if (found) {
      ...
      // Panel not in layout — add it to the active tabset
  ...
  const factory = useCallback((node: TabNode) => {
    ...
    if (!PanelComponent) {
      return (
        <div className="panel-full panel-body" style={{ color: "var(--text-muted)", padding: 16 }}>
