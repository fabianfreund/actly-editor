...
// Security: strictly validate the bin file name to prevent running arbitrary executables
fn validate_codex_bin(bin: &str) -> Result<(), String> {
    // Replace windows-style slashes with forward slashes to prevent bypass on Unix
    ...
    if file_name != "codex" && file_name != "codex.exe" {
        return Err("Executable must be named 'codex' or 'codex.exe'".to_string());
    }
    Ok(())
}
