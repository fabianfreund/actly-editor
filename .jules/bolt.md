...
## 2026-03-23 - Wrapping Timeline Event components in React.memo
**Learning:** Components rendering `FormattedText` can cause significant blocking on the main thread if they process markdown synchronously and constantly re-render due to parent state updates (like inputs).
**Action:** Wrapped `EventBubble` in `AgentThread` and `TimelineEvent` in `TaskDetail` inside `React.memo` and stabilized their callbacks (`handleApprovalDecision`, `onEventUpdated`) using `useCallback` to prevent unnecessary re-rendering on parent keystrokes.
